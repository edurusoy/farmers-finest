  ...
  export default function CustomerAddresses() {
    ...
    if(addressinfdata.length < 1){
      ...
      async function getCustomerAddresses() {
        const response = await fetch("/GetCustomerAddresses");
      ...
      async function postDeliveryAddressId(id) { 
        const obj={
            id: id
        };
        const response = await fetch("/SetDeliveryAddressId", {
          method: "POST",
          headers: {
      ...
      async function deleteCustomerAddresses() {
        const response = await fetch("/DeleteDeliveryAddresses");
        //const data = await response.json();
        console.log(response.status);
      ...
      const deleteHandler = (c)=>{
        postDeliveryAddressId(c.addressid);
      }
    ...
    <div className={classes.root}>
      <AppBar position="static" className={classes.styling}>
        ...
        <Link to={"/Customer"} className={classes.linkStyle}>
          ...
          <Grid item>
          <Link to={"/AddDeliveryAddress"} className={classes.linkStyle}>
          <Button variant ="outlined" color = "inherit" startIcon={<AddIcon/>} > New Delivery Address</Button>
          </Link>
          </Grid>
          <Grid item>
        ...
        <AccordionSummary aria-controls="panel1d-content" id="panel1d-header">
          <Typography>{`Delivery Address ${c.addressid}`}</Typography>
        </AccordionSummary>
        <AccordionDetails>
    <TableContainer component={Paper} className={classes.tableContainer}>
      <Table className={classes.table} aria-label="simple table">
        ...
        <Link to={`/DeliveryAddressUpdate/${c.addressid}`} className={classes.linkStyle}>
