...
async function postCreditCardId() { 
    const obj={
        id: id
    };
  ...
  async function getCreditCardwithId() {
    const response = await fetch("/GetCreditCardwithId");
...
async function postDataCreditCard(creditCard) {   
    const response = await fetch("/UpdateCreditCard", {
      method: "POST",
      headers: {
        "Content-type": "application/json; charset=UTF-8",
      },
