...
if(oldaddress.length < 1){
    postDeliveryAddressId();
}

async function postDeliveryAddressId() { 
    const obj={
        id: id
    };
  ...
  async function getDeliveryAddresses() {
    const response = await fetch("/GetDeliveryAddresswithId");
  ...
  async function postDataDeliveryAddress(address) {   
    const response = await fetch("/UpdateDeliveryAddress", {
      method: "POST",
      headers: {
        "Content-type": "application/json; charset=UTF-8",
      },
